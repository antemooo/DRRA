...
# In this part, what we need to achieve is to clean all the punctuations, digits and emjo in the tweet.
# also, need to clean the 'http' in each tweet:
...
# Method 2: get the old_data set for the further use:
# In this part, we need to use 2 methods separately to achieve noisy clean, so it will give us 2 results:
# one is achieved via kmeans and consensus matrix.
# Another method is achieved via DBSCAN:

# At first we need to achieve TF-IDF method to get the percent of each words and can be used for the further use
# get the combined tweet content into one list, this method is to get all the words in a file for TF-IDF.
# the method can be found on clean_tweet.py
...
# This step is to clean the noise inside the consensus matrix by using the idea in the assignment pdf.
# The method can be found in consensus_matrix.py.
# The result is a list with 0 and 1s, 1 means this tweet is a noise and should be removed.
noise_kmeans = consensus_noise(consensus_matrix)
# Using the noise vector we get from the previous step.
# We attach it into the tweets which has been cleaned from useless words or punctuation.
# We get a clean tweet that has no noise.
# This method can be found on consensus_matrix.py
tweets_kmeans_clean = clean_tweet_noise(tweet_clean, noise_kmeans)

##################################
# DBSCAN

# This method is to get the DBSCAN matrix by running DBSCAN 11 times.
# The number of elements in the range is from 5 to 15
db = DBSCAN_matrix(X_new, 5, 15)
# This step is to clean the noise inside the consensus matrix by using the idea in the assignment pdf.
# The method can be found in consensus_matrix.py.
# The result is a list with 0 and 1s, 1 means this tweet is a noise and should be removed.
db_noise = db_noise(db, threshold = 0.5)
# Using the noise vector we get from the previous step.
# We attach it into the tweets which has been cleaned from useless words or punctuation.
# We get a clean tweet that has no noise.
# This method can be found on consensus_matrix.py
...
# In this part, we used the results we got from the tweet old_data we cleaned.
# we generated 2 consensus matrix from the old_data we got via KMeans with consensus matrix and DBSCAN:
...
# Get the consensus matrix by using kmeans which k from 2 to 12, this consensus matrix is from the old_data via kmeans
...
# Get the consensus matrix by using KMeans which k from 2 to 12, this consensus matrix is from the old_data via DBSCAN
consensus_matrix2 = consensus_cluster(X_db_new, 2, 12)
# Generate a KMeans method for the first Consensus Matrix(the KMean and Consensus Matrix one).
# the KMeans is from sklearn
...
# Generate the cluster group, which is clusters for each tweets, this method can be found on cluster.py,
# the results are 9 csv files, which saved the tweets that been clustered in a group
...
# Generate nodes and edges for 2 methods.
# The information are in 2 csv files:
# one saved nodes of each cluster.
# the second stores edges that are connecting the nodes in the cluster.
# these files are used for Gephi
